...
def _load(name: str) -> Optional[Dict]:
    with db.read() as conn:
        ...
        if row is None:
            return None
        files = conn.execute(
            "SELECT * FROM files WHERE dataset_name=? ORDER BY relative_path", (name,)
        ).fetchall()
    ds = dict(row)
...
def _set_status(name: str, status: str, error: Optional[str] = None) -> None:
    with db.write() as conn:
        conn.execute(
            "UPDATE datasets SET status=?, error=?, updated_at=datetime('now') WHERE name=?",
            (status, error, name),
        )


def _set_dataset_id(name: str, dataset_id: str) -> None:
    with db.write() as conn:
        conn.execute("UPDATE datasets SET dataset_id=? WHERE name=?", (dataset_id, name))


def _update_file(name: str, rel: str, **fields) -> None:
    cols = ", ".join(f"{k}=?" for k in fields)
    with db.write() as conn:
        conn.execute(
            f"UPDATE files SET {cols} WHERE dataset_name=? AND relative_path=?",
            (*fields.values(), name, rel),
        )


def transfer_dataset(
    gateway: CirroGateway, name: str, default_project: Optional[str], emit: Emit
) -> str:
    ...
    def status(new: str, **extra) -> None:
        _set_status(name, new, error=extra.get("error"))
        emit({"type": "dataset", "name": name, "status": new, **extra})

    try:
        project_ref = ds["project"] or default_project
        if not project_ref:
            raise ValueError("No project set for this dataset (CSV or UI default required)")
        ...
        staging = config.staging_root / _safe_dirname(name)
        ...
        for f in ds["files"]:
            rel = f["relative_path"]
            dest = staging / rel
            _update_file(name, rel, status=Status.DOWNLOADING, downloaded_bytes=0)

            state = {"n": 0}

            def on_bytes(delta: int, rel=rel, state=state) -> None:
                state["n"] += delta
                emit({
                    "type": "progress", "name": name, "phase": "download",
            ...
            _update_file(
                name, rel, status="DONE", downloaded_bytes=result.size,
...
def _upload(
    ...
    if resume:
        ...
        emit({"type": "progress", "name": name, "phase": "upload", "resume": True, "total": total})
        gateway.upload_files(project_id, dataset_id, str(staging), rel_paths, resume=True)
        emit({"type": "progress", "name": name, "phase": "upload", "done": total, "total": total})
        return
    ...
    for i, rel in enumerate(rel_paths, start=1):
        ...
        emit({"type": "progress", "name": name, "phase": "upload",
